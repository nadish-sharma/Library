...
function SearchFiltersModal({url, onUpdateUrl, filter, onUpdateFilter, search , onClose }) {
        ...
        <div className="modal-body">
          <ul className="modal-body-list">
            {SearchFilterItems.map((item, index) => {
              return (
                <li key={index} className="search-list-body" onClick={handleFilterClick}>
                  {/* <Link className={item.cName} to={item.url}> */}
                  <button className={item.cName} onClick={ handleFilterClick}>{item.title}</button><span>{item.description}</span>
                  {/* </Link> */}
                </li>);
            })}
